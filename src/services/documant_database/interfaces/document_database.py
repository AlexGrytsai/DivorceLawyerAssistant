...
class DocumentDatabase(ABC, Generic[DocumentType]):
    ...
    @abstractmethod
    async def save(
        self, collection: str, document: DocumentType
    ) -> str:
        """
    ...
    @abstractmethod
    def serialize(self, document: DocumentType) -> Dict[str, Any]:
        """
    ...
    @abstractmethod
    def deserialize(self, data: Dict[str, Any]) -> DocumentType:
        """
        Convert dictionary data to document format.
